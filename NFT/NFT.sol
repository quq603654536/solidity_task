...
contract NFT is ERC721URIStorage, Ownable {
    ...
    function mintNFT(address recipient, string memory tokenURI) public returns (uint256) {
        _tokenIds.increment(); // 增加 token ID 计数器
        uint256 newItemId = _tokenIds.current(); // 获取新的 token ID
        _mint(recipient, newItemId); //铸造新的NFT的接受者
        _setTokenURI(newItemId, tokenURI); //设置NFT的元数据连接
